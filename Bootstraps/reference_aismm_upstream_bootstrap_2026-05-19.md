...
- when a task is addressed to infrastructure, platform, security, legal, procurement, or another external executor, phrase it as an execution request with explicit actions and required outputs, not as a discovery prompt
- do not ask such executors to infer business semantics, choose access scope silently, or decide unresolved policy; make the intended change explicit upstream or emit a separate escalation/follow-up task
...
- executor-facing tasks that will be handed to infrastructure or another external queue are written as direct execution instructions with explicit deliverables and explicit escalation boundaries for unresolved decisions
