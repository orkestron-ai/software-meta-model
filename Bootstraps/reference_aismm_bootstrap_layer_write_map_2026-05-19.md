...
- when such a `task` is addressed to infrastructure, platform, security, legal, procurement, or another external queue, write it as a direct execution request with explicit outputs; unresolved scope or policy choices must be separate escalations, not hidden inside the task wording
